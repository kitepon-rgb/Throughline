...
- 各レコードは `origin_session_id` を保持するため、複数回の `/clear` を跨いで記憶がチェーン状に蓄積する（ホップ制限なし）
- 並行セッション時は「同プロジェクト内で最後に Claude が反応したセッション」を前任として引き継ぐ（注入ヘッダで明示）
...
| ファイル | 役割 |
|---|---|
| [src/session-merger.mjs](src/session-merger.mjs) | `resolveMergeTarget` / `mergePredecessorInto`（BEGIN IMMEDIATE トランザクション） |
...
| ファイル | 対象 |
|---|---|
| [src/session-merger.test.mjs](src/session-merger.test.mjs) | `resolveMergeTarget` / `mergePredecessorInto` |
