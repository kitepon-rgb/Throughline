...
| **未公開: `/tl-trim` dry-run** | `throughline trim --dry-run`、`--host`、`--keep-recent`、`--all`、`--memo-stdin`、`--codex-thread-id`、`throughline doctor --trim`、Claude slash command `/tl-trim` を追加。Codex app-server の rollback / inject primitive は実測済みだが、Throughline CLI から対象 thread を安全に制御する統合は未実装のため automatic rollback / inject は無効。curated memory は current-work memo と active work thread framing を含み、L2 を単なる過去ログとして読ませない |
