...
async function createReleaseFixture(t) {
  const root = await mkdtemp(path.join(tmpdir(), "throughline-release-gate-"));
  ...
  git(work, "config", "user.name", "Throughline release gate test");
  git(work, "config", "user.email", "throughline-release-gate@example.invalid");
