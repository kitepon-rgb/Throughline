...
> **Implementation status**: L1 (skeletons) and L2 (bodies) write paths are
> working. **L3 (details) write path is not yet implemented** — the slash
> command `/sc-detail` returns L2 correctly but its L3 section is always empty.
> See [docs/L1_L2_L3_REDESIGN.md](docs/L1_L2_L3_REDESIGN.md) for the full spec
> and the remaining work items.
...
- `details` — L3 tool I/O (schema ready, write path pending)
...
**`/sc-detail` returns L2 but no L3**
Known issue. L3 write path (tool I/O capture) is not yet implemented in the
current refactor. See [docs/L1_L2_L3_REDESIGN.md](docs/L1_L2_L3_REDESIGN.md)
step 4 for the remaining work.
