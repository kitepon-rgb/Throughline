...
| [src/cli/codex-handoff-start.mjs](src/cli/codex-handoff-start.mjs) | `codex-handoff-start` — 新規 Codex thread へ移るための guided surface。structural smoke、model smoke dry-run boundary、handoff render command、optional live smoke command、`--print-prompt` の結合済み handoff prompt をまとめて出す。`--execute` では `codex app-server thread/start` + `thread/inject_items` で新 thread に developer memory を注入し、`--open-host auto\|desktop\|vscode\|cli\|none` で表示を開く。`auto` は Codex Desktop 起点を識別して `codex://threads/<thread-id>` で同アプリ内に開き、VS Code / CLI の既存経路を維持する。`--memo-stdin` 時は replay 用の個別 command にも `--memo-stdin` を伝播し、same memo を pipe する注意を出す。current thread は mutate しない |
