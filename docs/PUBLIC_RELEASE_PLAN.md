...
| **未公開: `/tl-trim` dry-run** | `throughline trim --dry-run`、`--preflight`、experimental `--execute`、`--host`、`--keep-recent`、`--all`、`--memo-stdin`、`--codex-thread-id`、`THROUGHLINE_CODEX_THREAD_ID` / `CODEX_THREAD_ID`、`throughline codex-threads`、`throughline doctor --trim`、Claude slash command `/tl-trim` を追加。Codex app-server の rollback / inject primitive は実測済みで、`--execute` は Codex thread id と `THROUGHLINE_EXPERIMENTAL_CODEX_TRIM=1` がある場合だけ rollback / inject を送る。automatic rollback / inject は引き続き無効。curated memory は current-work memo と active work thread framing を含み、L2 を単なる過去ログとして読ませない |
