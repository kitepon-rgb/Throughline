...
| [src/codex-auto-refresh.test.mjs](src/codex-auto-refresh.test.mjs) | 80% 閾値、estimate usage の非実行、threshold reached で auto-refresh が rollback/inject を呼ぶこと、DB memory が無い場合の skip |
...
- Codex UserPromptSubmit / PostToolUse hooks は token-monitor に依存しない。hook 自身が当該 Codex session の rollout `token_count` を読み、verified 80% 以上なら同じ user turn または tool loop 継続前に `$throughline` workflow 実行指示を `additionalContext` で注入する。これは monitor を起動していないユーザーでも動く current-session trigger であり、モニターの表示値を自動発火の根拠にしない。
...
- Codex Stop hook 後の automatic refresh mutation は、verified usage が `80%` 以上なら rollback / inject を送る。estimate usage または estimated context window では実行しない。Codex native auto-compact より先に Throughline memory refresh を走らせつつ、70% warning よりは mutation を遅らせる。
...
- **Claude PostToolUse** は登録しない（schema v4 で廃止）。Codex PostToolUse は別用途で、tool loop 中の 80% current-session refresh instruction hook として登録する。
