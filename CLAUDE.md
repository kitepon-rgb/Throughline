...
| [src/codex-auto-refresh.mjs](src/codex-auto-refresh.mjs) | Codex Stop hook 後の automatic refresh 判定。verified usage が 80% 以上なら rollout source と Throughline DB memory を確認し、guarded rollback + Throughline DB memory inject を送る。estimate usage / estimated context window では実行しない |
